...
void BllRealTimeTrans1::clientLogin()
{
	...
	if (success)
	{
		...
		Global::systemLog->append(QString(tr("信息")), QString("服务器") + QString::number(serverNo)
			+ QString(tr("服务器：登陆成功")), SystemLog::INFO_TYPE);
	...
	else
	{
		...
		//写入系统日志
		Global::systemLog->append(QString(tr("信息")), QString("服务器") + QString::number(serverNo)
			+ QString(tr("登陆错误")), SystemLog::INFO_TYPE);
...
void BllRealTimeTrans1::requestRaceID( int session )
{
	...
	else
	{
		...
		//写入系统日志
		Global::systemLog->append(QString(tr("信息")), QString("服务器") + QString::number(serverNo)
			+ QString(tr("识别端：错误，请求RaceID指令失败")), SystemLog::INFO_TYPE);
...
void BllRealTimeTrans1::requestRaceIDForKeepAlive(int session)
{
	...
	else
	{
		...
		//写入系统日志
		Global::systemLog->append(QString(tr("信息")), QString("服务器") + QString::number(serverNo)
			+ QString(tr("识别端：为保持连接请求RaceID失败")) , SystemLog::INFO_TYPE);
...
void BllRealTimeTrans1::handleRequestRaceID(QByteArray result, int descriptor)
{
	...
	//写入系统日志
	Global::systemLog->append(QString(tr("信息")), QString("服务器") + QString::number(serverNo)
		+ QString(tr("服务端：请求raceID成功。回复，raceID"))
		+ QString::number(raceId), SystemLog::INFO_TYPE);
...
void BllRealTimeTrans1::submitRaceTime(qint32 raceTime)
{
	...
	if (success)
	{
		...
		//写入系统日志
		Global::systemLog->append(QString(tr("信息")),QString("服务器")+QString::number(serverNo)
			+ QString(tr("服务器１　：　服务端：回复，提交比赛时长"))
			+ raceTimeStr, SystemLog::INFO_TYPE);

		emit statuChanged(QString("服务器1：　识别端：提交比赛时长： %1 指令 。").arg(raceTimeStr) +
...
void BllRealTimeTrans1::handleSubmitRaceTime(QByteArray result, int descriptor)
{
	...
	{
		emit statuChanged(QString("服务器 %2,服务端：未回复，未接收到比赛时长指令，%1").arg(reply).arg(serverNo));
...
*/
int BllRealTimeTrans1::submitWINOrPLA(DataOutput& ouputStruct, QString type)
{
	TagProtocolMsg msg;
	msg.MsgID = 8;
...
* @param ouputStruct：算法传递来的数据，type:传送类型默认win
*/
int BllRealTimeTrans1::submitQINOrQPL(DataOutput &ouputStruct, QString type)
{
	TagProtocolMsg msg;
	msg.MsgID = 8;
	...
	else
	{
		...
		emit statuChanged(QString("服务器 %2 识别端错误，提交实时数据指令 -%1,失败.").arg(type).arg(serverNo));
...
void BllRealTimeTrans1::submitRaceEnd()
{
	TagProtocolMsg msg;
	memset(&msg, 0, 64);
	strcpy(msg.Check, "RDBS");
