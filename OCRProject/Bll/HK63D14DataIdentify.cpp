...
HK63D14DataIdentify::HK63D14DataIdentify()
{
	dataLength = 0;
	imageHeight = 0;
	imageWidth = 0;
...
void  HK63D14DataIdentify::isReady()
{
	;
}
...
void HK63D14DataIdentify::haveData()
{
...
void HK63D14DataIdentify::originPosition()
{
...
int HK63D14DataIdentify::identify()
{
...
/*
	设置 马名的rect区间
*/
int HK63D14DataIdentify::setHorseNameRectPos()
{
	Mat horseNameRegion(image, HORSENAME_REGION_RECT_HK63D14);
 ...
 //如果 函数执行失败，那么退出此时识别
	...
	{
		horseNamePosStruct.rect[r].x = HORSENAME_REGION_RECT_HK63D14.x;
		horseNamePosStruct.rect[r].width = HORSENAME_REGION_RECT_HK63D14.width;

		horseNamePosStruct.rect[r].y = HORSENAME_REGION_RECT_HK63D14.y + y[r];
		if (r == dataOutput.horseNum - 1)
		{
			//防止高度太高
			horseNamePosStruct.rect[r].height = HORSENAME_REGION_RECT_HK63D14.height - y[r];
...
int HK63D14DataIdentify::setWINPLARectPos()
{
	...
	Mat winRegion(imageGray, WIN_POS_RECT_HK63D14);
	Mat plaRegion(imageGray, PLA_POS_RECT_HK63D14);
	...
	{
		// 3个像素 空白	 		
		winPlaPosStruct.rect[i][0].x = WIN_POS_RECT_HK63D14.x + x0 - 2;
		winPlaPosStruct.rect[i][0].width = x1 - x0 + 4;

		
		winPlaPosStruct.rect[i][0].y = WIN_POS_RECT_HK63D14.y + yWin[i];

		if (i == dataOutput.horseNum - 1)
		{
			
			winPlaPosStruct.rect[i][0].height = NUMBER_HEIGHT_HK63D14 ;
		}
		else
		{
			...
			{
		//		yWin[i + 1] = yWin[i] + NUMBER_HEIGHT_HK63D14 + 2;
			}
		...
		{
			winPlaPosStruct.rect[i][0].width = NUMBER_WIDTH_HK63D14;
		}
	...
	{
		// 3个像素 空白	 		
		winPlaPosStruct.rect[i][1].x = PLA_POS_RECT_HK63D14.x + x0 - 2;
		winPlaPosStruct.rect[i][1].width = x1 - x0 + 4;

		winPlaPosStruct.rect[i][1].y = PLA_POS_RECT_HK63D14.y + yPla[i];


		if (i == dataOutput.horseNum - 1)
		{
			winPlaPosStruct.rect[i][1].height = NUMBER_HEIGHT_HK63D14;
		}
		else
		{
		...
		{
			winPlaPosStruct.rect[i][1].width = NUMBER_WIDTH_HK63D14;
		}
...
int HK63D14DataIdentify::setQINQPLRectPos()
{
#ifdef QDEBUG_OUTPUT
...
//获取一列QIN QPL 位置
	...
	{
		qinQplSubRect[0] = LB_REGION1_RECT_HK63D14;
		qinQplSubRect[1] = LB_REGION2_RECT_HK63D14;
		qinQplSubRect[2] = LB_REGION3_RECT_HK63D14;
		qinQplSubRect[3] = LB_REGION4_RECT_HK63D14;
		qinQplSubRect[4] = LB_REGION5_RECT_HK63D14;
		qinQplSubRect[5] = LB_REGION6_RECT_HK63D4;

		qinQplSubRect[6] = RU_REGION1_RECT_HK63D14;
		qinQplSubRect[7] = RU_REGION2_RECT_HK63D14;
		qinQplSubRect[8] = RU_REGION3_RECT_HK63D4;
		qinQplSubRect[9] = RU_REGION4_RECT_HK63D14;
		qinQplSubRect[10] = RU_REGION5_RECT_HK63D14;
		qinQplSubRect[11] = RU_REGION6_RECT_HK63D14;


		qinQplSubRect[12] = R_REGION1_RECT_HK63D14;
		qinQplSubRect[13] = R_REGION2_RECT_HK63D14;
		qinQplSubRect[14] = R_REGION3_RECT_HK63D14;
		qinQplSubRect[15] = R_REGION4_RECT_HK63D14;
		qinQplSubRect[16] = R_REGION5_RECT_HK63D14;
		qinQplSubRect[17] = R_REGION6_RECT_HK63D14;
		qinQplSubRect[18] = R_REGION7_RECT_HK63D14;
	...
	Mat qinQPLPosStructRegionSub1(image_temp, QINQPL_POS_RECT_HK63D14 );
...
//输入canny 处理后图像
int  HK63D14DataIdentify::setEveryQINQPLPos(Mat &mat, int rectNum)
{
#ifdef QDEBUG_OUTPUT
	...
	if (rectNum < 6 - delataNum) // 6-1
	{
		...
		for (int i = rectNum + 1; i < 7 - delataNum; i++)
		{
			...
			if (i == 6 - delataNum)
			{
				...
				qinQPLPosStruct.rect[i][rectNum].height = NUMBER_HEIGHT_HK63D14 ;
			}
			else
			{
				if (y[i - rectNum] - y[i - rectNum - 1] > 22 )
				{
					y[i - rectNum] = y[i - rectNum - 1] + NUMBER_HEIGHT_HK63D14  ;
				}
	...
	else if (rectNum >= 6 & rectNum < 12)
	{
		...
		for (int i = 0; i <roiNum; i++)
		{
			...
			if (i == roiNum - 1)
			{
				...
				qinQPLPosStruct.rect[i][rectNum - 4].height = NUMBER_HEIGHT_HK63D14 ;
			}
			else
			{
				if (y[i + 1] - y[i ] > 22 )
				{
					y[i +1] = y[i ] + NUMBER_HEIGHT_HK63D14  ;
				}
...
int HK63D14DataIdentify::setSessionRectPos()
{
	if (algorithmState == EXIT_THIS_OCR)
	{
		return EXIT_THIS_OCR;
	...
	sessionPosStruct.rect[0] = SESSION_POS_RECT_HK63D14;
 
	return EXEC_SUCCESS;
}

//设置 倒计时位置 
int HK63D14DataIdentify::setCountDownRectPos()
{
	if (algorithmState == EXIT_THIS_OCR)
	{
		return EXIT_THIS_OCR;
	...
	// 十位数
	countDownMinutePosStruct.rect[0] = COUNTDOWNMINUTE_POS_RECT1_HK63D14;
	//个位数
	countDownMinutePosStruct.rect[1] = COUNTDOWNMINUTE_POS_RECT2_HK63D14;

	return EXEC_SUCCESS;
}
//识别马名 
int HK63D14DataIdentify::getHorseNameIdentify()
{
...
//通过计算灰度值和，来确定是否发生了马名字的变化，马名字发生了变化，说明
int HK63D14DataIdentify::isHorseNameChanged()
{
...
int HK63D14DataIdentify::calculateGraySum(Mat srcMat,int &length)
{
	int graySum = 0 ;
...
int HK63D14DataIdentify::getWINPLAIdentify()
{
	if (algorithmState == EXIT_THIS_OCR)
	{
		return EXIT_THIS_OCR ;
...
//检测数字底下是否有 绿色底色
// srcMat彩色原图
bool HK63D14DataIdentify::haveGroundColor(Mat srcMat ,int flag )
{
	//转为灰度图像
...
//识别 QIN QPL   同时识别ＱＩＮ　ＱＰＬ
int HK63D14DataIdentify::getQINQPLIdentify()
{
	...
	int dotFlag;
	Mat qinQPLPosStructMat(imageTemp, WHOLE_QINQPL_POS_RECT_HK63D14_HK63D14);
	Mat qinQPLPosStructRegionSub1(imageTemp, QINQPL_POS_RECT_HK63D14);
...
int HK63D14DataIdentify::getSessionIdentify()
{
...
//识别倒计时 
int HK63D14DataIdentify::getCountDownIdentify()
{
...
int HK63D14DataIdentify::calculateGraySumXForSetWINPLARect(Mat &mat, int *y, int &horseNum)
{
...
int HK63D14DataIdentify::calculateGraySumXForSetHorseNameRect(Mat &mat, int *y, int &horseNum)
{
...
bool HK63D14DataIdentify::judgeWINPLADot(int i, Mat &edge, Mat &roiThresholdEdge)
{
...
int  HK63D14DataIdentify::judgeQINQPLDot(Mat &roi, Mat &edge, int *x)
{
...
int HK63D14DataIdentify::judgeQINQPL()
{
	if (algorithmState == EXIT_THIS_OCR )
	{
		return EXIT_THIS_OCR ;
	...
	Mat roi(image, QINQPL_LABEL_POS_RECT_HK63D14);
...
裁剪ROI区域多余的空白像素
*/
int  HK63D14DataIdentify::trimRoiBlankPart(Mat &oriMat, Mat &newRoiMat, CvRect &roiNewSize)
{
	if (algorithmState == EXIT_THIS_OCR)
	{
		return EXIT_THIS_OCR;
...
int HK63D14DataIdentify::colorThreshold(Mat &src,Mat &dst, int thereshold)
{
	if (algorithmState == EXIT_THIS_OCR )
	{
		return EXIT_THIS_OCR;
...
int  HK63D14DataIdentify::calculateXBewttenNumber(Mat &mat, int  *x )
{
	if (mat.empty())
	{
#ifdef  QDEBUG_OUTPUT
...
int  HK63D14DataIdentify::calculateGraySumYForQINDotJudge(Mat &mat, int  *x, int roiNum )
{
...
//计算纵向投影
int  HK63D14DataIdentify::calculateGraySumYForTrim(Mat &mat, int &x0, int &x1, int roiNum)
{
	if (mat.empty()| algorithmState == EXIT_THIS_OCR)
	{
...
int  HK63D14DataIdentify::calculateGraySumXForSetQINQPLRect(Mat &mat, int  *y, int roiNum )
{
...
*/
void HK63D14DataIdentify::createClassifySamples(float result, Mat &singleNum)
{
	//重新获取目录
...
void HK63D14DataIdentify::writeSamples(QString fileName, Mat &roi,QString path)
{
	//重新获取目录
	QString runPath = QCoreApplication::applicationDirPath();
